@permission('edit.ditch.inventory_roads')

<div class="modal-content" id="ditch_edit">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">
            <span aria-hidden="true">×</span>
        </button>
        <h4 class="modal-title" id="myModalLabel">
            <i class="fa fa-automobile"></i> {{ trans('ditch.labels.edit') }}
        </h4>
    </div>
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_content">
            <form role="form" action="{{ route('update.edit.ditch.inventory_roads', ['gid' => $entity->gid]) }}"
                  method="post"
                  class="form-horizontal form-label-left" id="ditch_edit_fm">
                @csrf
                        ...
                        <select class="form-control select2"
                            ...
                            @foreach($typeDitches as $typeDitche)
                                <option value="{{ $typeDitche->descrip }}"
                                        @if($entity->tipo == $typeDitche->descrip) selected @endif>
                                    {{ $typeDitche->descrip }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <textarea name="observ" id="observ" class="form-control col-md-7 col-sm-7 col-xs-12" rows="5"
                                  placeholder="{{ trans('ditch.placeholders.observ') }}">{{ $entity->observ }}</textarea>
                    </div>
                </div>
