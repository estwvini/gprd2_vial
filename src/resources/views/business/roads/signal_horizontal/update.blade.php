@permission('edit.signal_horizontal.inventory_roads')

<div class="modal-content" id="signal_horizontal_edit">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">
            <span aria-hidden="true">×</span>
        </button>
        <h4 class="modal-title" id="myModalLabel">
            <i class="fa fa-automobile"></i> {{ trans('signal_horizontal.labels.edit') }}
        </h4>
    </div>
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_content">
            <form role="form"
                  action="{{ route('update.edit.signal_horizontal.inventory_roads', ['gid' => $entity->gid]) }}"
                  method="post"
                  class="form-horizontal form-label-left" id="signal_horizontal_edit_fm">
                @csrf
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <select class="form-control select2"
                            ...
                            @foreach($typeHorizontalSignals as $typeHorizontalSignal)
                                <option value="{{ $typeHorizontalSignal->descrip }}"
                                        @if($entity->tipo == $typeHorizontalSignal->descrip) selected @endif>
                                    {{ $typeHorizontalSignal->descrip }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <textarea name="observ" id="observ" class="form-control col-md-7 col-sm-7 col-xs-12" rows="5"
                                  placeholder="{{ trans('signal_horizontal.placeholders.observ') }}">{{ $entity->observ }}</textarea>
                    </div>
                </div>
...
<script>
    $(() => {
        ...
        let $form = $('#signal_horizontal_edit_fm');
